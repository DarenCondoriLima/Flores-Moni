...
function addSkyText(message) {
    const garden = document.querySelector(".garden");
    
    // Evita duplicar el texto si ya existe uno
    if (garden.querySelector(".sky-text")) return;

    const skyText = document.createElement("div");
    skyText.classList.add("sky-text");
    skyText.textContent = message;

    garden.appendChild(skyText);
}
  
	function createGarden() {
		const flowerCount = 20;

  addSkyText("Monica, te amo ❤️");
	...
	function createFlower(
		...
		// Se limpiaron los caracteres invisibles en el gradiente:
		stem.style.background = "linear-gradient(to top, rgba(30, 20, 10, 0.9) 0%, #2e5d20 5%, #5cad4a 50%, #2e5d20 95%)";
